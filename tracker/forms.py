...
class ProjectForm(forms.ModelForm):
    ...
    class Meta:
        model = Project
        fields = ["name", "description", "status", "start_date", "target_date", "owner"]
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            FormActions(
                ...
                HTML(
                    '<a href="{% url "tracker:project_list" %}" class="btn btn-secondary ms-2">Cancel</a>'
                ),
            ),
        )
    ...
    def clean(self):
        ...
        if start_date and target_date:
            if target_date <= start_date:
                raise ValidationError("Target date must be after the start date.")
...
class ApplicationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "status": forms.Select(attrs={"class": "form-select"}),
            "estimated_weeks": forms.NumberInput(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Fieldset(
                ...
                "features_text",
                HTML(
                    '<small class="text-muted">Suggested weeks based on complexity: Simple (2-4), Medium (4-8), High (8-16)</small>'
                ),
            ),
            FormActions(
                Submit("submit", "Save Application", css_class="btn btn-primary"),
                HTML(
                    '<a href="{% url "tracker:application_list" %}" class="btn btn-secondary ms-2">Cancel</a>'
                ),
            ),
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Fieldset(
                ...
                HTML(
                    '<div class="alert alert-info"><i class="bi bi-info-circle me-2"></i><strong>Required:</strong> Name and Content</div>'
                ),
                "name",
                "content",
                HTML('<hr class="my-3">'),
                ...
                "file_upload",
                HTML(
                    '<small class="text-muted">Supported formats: PDF, DOC, DOCX, TXT, PY, JS, HTML, CSS, MD (Max 10MB)</small>'
                ),
                "description",
            ),
            FormActions(
                ...
                HTML(
                    '<a href="{% url "tracker:artifact_list" %}" class="btn btn-secondary ms-2">Cancel</a>'
                ),
            ),
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Fieldset(
                ...
                "actual_hours",
                HTML(
                    '<small class="text-muted">Assignee options: Claude (AI assistance), GitHub Copilot (Code generation), Human (Manual tasks), Team (Collaborative work)</small>'
                ),
            ),
            FormActions(
                Submit("submit", "Save Task", css_class="btn btn-primary"),
                HTML(
                    '<a href="{% url "tracker:task_list" %}" class="btn btn-secondary ms-2">Cancel</a>'
                ),
            ),
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            FormActions(
                ...
                HTML(
                    '<a href="{% url "tracker:search" %}" class="btn btn-outline-secondary ms-2">Clear</a>'
                ),
            ),
        )
...
class BulkTaskForm(forms.Form):
    ...
    selected_tasks = forms.CharField(widget=forms.HiddenInput(), required=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            FormActions(
                ...
                HTML(
                    '<a href="{% url "tracker:decision_list" %}" class="btn btn-secondary ms-2">Cancel</a>'
                ),
            ),
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            FormActions(
                ...
                HTML(
                    '<a href="{% url "tracker:integration_list" %}" class="btn btn-secondary ms-2">Cancel</a>'
                ),
            ),
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Fieldset(
                ...
                HTML(
                    '<div class="alert alert-info"><i class="bi bi-info-circle me-2"></i><strong>Claude Artifact Style:</strong> Content will be formatted with Markdown for professional display</div>'
                ),
                "name",
                "content",
            ),
            FormActions(
                ...
                HTML(
                    '<a href="{% url "tracker:requirement_list" %}" class="btn btn-secondary ms-2">Cancel</a>'
                ),
            ),
        )
