...
def dashboard_view(request):
    ...
    recent_artifacts = Artifact.objects.filter(
        application__project__in=projects_queryset, updated_at__gte=cutoff_date
    ...
    pending_decisions = Decision.objects.filter(
        project__in=projects_queryset, status="pending"
    ...
    monthly_data = (
        ...
        .values("month")
        .annotate(
            completed=Count("id", filter=Q(status="completed")), total=Count("id")
        )
        .order_by("month")
    )
    ...
    status_distribution = (
        projects_queryset.values("status")
...
class ProjectListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Project.objects.all().prefetch_related(
            "applications", "applications__tasks"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["owners"] = Project.objects.values(
...
class ProjectDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Pending decisions
        pending_decisions = project.decisions.filter(status="pending").order_by(
            "decided_date"
        )
        ...
        context.update(
            {
                ...
                "timeline_data": sorted(
                    timeline_data, key=lambda x: x["date"] or timezone.now().date()
                ),
                "pending_decisions": pending_decisions,
            }
        )
...
class ProjectUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Project "{form.instance.name}" updated successfully!'
        )
        return super().form_valid(form)
...
def project_dashboard_view(request, pk):
    ...
    # App status summary
    app_status = (
        project.applications.values("status")
        .annotate(count=Count("id"))
        .order_by("status")
    )
...
class ApplicationListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Application.objects.select_related("project").prefetch_related(
            "tasks", "artifacts"
        )
...
class ApplicationDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for status_code, status_display in Task.STATUS_CHOICES:
            tasks_by_status[status_display] = application.tasks.filter(
                status=status_code
            )
...
class ApplicationCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Application "{form.instance.name}" created successfully!'
        )
        return super().form_valid(form)
...
class ApplicationUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Application "{form.instance.name}" updated successfully!'
        )
        return super().form_valid(form)
...
class ArtifactListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Artifact.objects.select_related(
            "application", "application__project"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        version_history = (
            Artifact.objects.filter(
                application=artifact.application, name=artifact.name
            )
...
class ArtifactCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Artifact "{form.instance.name}" created successfully!'
        )
        return super().form_valid(form)
...
class ArtifactUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Artifact "{form.instance.name}" updated successfully!'
        )
        return super().form_valid(form)
...
def artifact_download_view(request, pk):
    ...
    response = HttpResponse(
        artifact.file_upload.read(), content_type="application/octet-stream"
    )
    response["Content-Disposition"] = (
        f'attachment; filename="{artifact.file_upload.name}"'
    )
    return response
...
class TaskListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET.get("view") == "kanban":
            tasks_by_status = {}
            for status_code, status_display in Task.STATUS_CHOICES:
                tasks_by_status[status_display] = self.get_queryset().filter(
                    status=status_code
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if action in ["complete", "in_progress", "pending"]:
                ...
                messages.success(
                    request, f"{updated_count} tasks updated to {action} status."
                )
...
class IntegrationListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for status_code, status_display in Integration.STATUS_CHOICES:
            integrations_by_status[status_display] = self.get_queryset().filter(
                status=status_code
            )
...
def search_view(request):
    ...
    if form.is_valid():
        ...
        if search_type in ["all", "integrations"]:
            ...
            if project_filter:
                integrations_q &= Q(from_app__project=project_filter) | Q(
                    to_app__project=project_filter
                )
...
def api_chart_data(request):
    ...
    if chart_type == "monthly_tasks":
        # Monthly task completion data
        data = (
            Task.objects.annotate(month=TruncMonth("updated_at"))
            .values("month")
            .annotate(
                completed=Count("id", filter=Q(status="completed")), total=Count("id")
            )
...
class DecisionCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Decision "{form.instance.title}" created successfully!'
        )
        return super().form_valid(form)
...
class DecisionUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Decision "{form.instance.title}" updated successfully!'
        )
        return super().form_valid(form)
...
class RequirementCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Requirement "{form.instance.name}" created successfully!'
        )
        return super().form_valid(form)
...
class RequirementUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'Requirement "{form.instance.name}" updated successfully!'
        )
        return super().form_valid(form)
