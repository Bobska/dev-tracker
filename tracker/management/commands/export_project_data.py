...
class Command(BaseCommand):
        ...
        try:
            ...
            if options["project"]:
                try:
                    ...
                    raise CommandError(
                        f'Project with ID {options["project"]} not found'
                    )
            else:
    ...
    def get_filtered_data(self, project, include_types):
        ...
        if "artifacts" in include_types:
            if project:
                data["artifacts"] = list(
                    Artifact.objects.filter(application__project=project)
                )
            else:
        ...
        if "integrations" in include_types:
            if project:
                data["integrations"] = list(
                    Integration.objects.filter(from_application__project=project)
                )
            else:
    ...
    def export_json(self, output_path, project, include_types):
            ...
            if objects:
                ...
                for obj in objects:
                    ...
                    if hasattr(obj, "project"):
                        ...
                        obj_data["project_name"] = (
                            obj.project.name if obj.project else None
                        )
                    if hasattr(obj, "application"):
                        obj_data["application_id"] = (
                            obj.application.id if obj.application else None
                        )
                        obj_data["application_name"] = (
                            obj.application.name if obj.application else None
                        )
                        ...
                        obj_data.update(
                            {
                                ...
                                "due_date": (
                                    obj.due_date.isoformat() if obj.due_date else None
                                ),
                                "assigned_to": (
                        ...
                        obj_data.update(
                            {
                                ...
                                "version": obj.version,
                                "file_size": (
                                    obj.file_size if hasattr(obj, "file_size") else None
                                ),
                            }
                        )
        ...
        self.stdout.write(
            f'Exported {sum(len(v) for v in export_data["data"].values())} records'
        )
                        ...
                        writer.writerow(
                            {
                                ...
                                "application": (
                                    obj.application.name if obj.application else ""
                                ),
    ...
    def export_excel(self, output_path, project, include_types):
        ...
        try:
            ...
            raise CommandError(
                "openpyxl and pandas are required for Excel export. Install with: pip install openpyxl pandas"
            )
        ...
        self.stdout.write(
            f"Created Excel file with {len(data)} sheets and {total_records} total records"
        )
